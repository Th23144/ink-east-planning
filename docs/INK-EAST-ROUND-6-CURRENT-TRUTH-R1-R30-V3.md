# Round 6 — Identity / Role / Permission — Current Truth R1–R30 V3

Status: **CANDIDATE / NOT SEALED**. No implementation authorization. Do not enter Round 7.

This is the source-completeness-repaired one-file candidate. Earlier consolidations, the premature seal, reopen records and audit files remain historical provenance. This file carries accepted Round 6A–6J, applicable cross-round corrections, and R6-R1 through R6-R30.
...
Actual Actor and public attribution are distinct. Representative publishing, comments and replies require clear Acting Context. Do not use an invisible sticky Organization identity. Ordinary users default to personal/selected identity and do not see Organization/delegation/control machinery without a relevant relationship. Saves, follows, purchases, account security, preferences and comparable personal/private actions remain personal even when the same account can represent an Organization. Official Organization speech and personal opinion remain distinguishable. Author, Publisher and Operator may differ; commercial actions preserve the actual Operator plus the relevant acting/asset-owning subject. Multi-author/contributor relationships are first-class rather than one author_id.
...
Multiple valid authorization sources may coexist. Compute Effective Permission rather than highest Role. Conflicts resolve at Permission + Scope. Applicable security Deny/restriction overrides ordinary Allow within Scope. Consequential execution rechecks current authority/security context. Opening a page while authorized does not preserve stale authority for later commit. Revoking one path removes only authority from that path; independent valid grants remain unless separately affected. Revocation changes future authority and does not delete existing draft objects merely because current authority ended; access/editability of those drafts is decided by their own ownership/permission rules. Historical authorship/audit/provenance remains intact.
...
Account, Stable Platform Identity, Authority-bearing Subject and Relationship lifecycles are separate. Deactivate, Suspend, Close and Delete are distinct. Data disposition is category-specific rather than one universal delete flag and distinguishes at least authentication/security data, private personal data, public profile data, published Works, community activity, private communications, transactions, and audit/security records. Account closure does not automatically erase public Works, citations, Organization history or provenance, while private-data minimization/deletion remains separately governed. Inactivity != death/fraud/abandonment/entitlement transfer and does not by itself trigger default deletion of public knowledge.
...
This V3 candidate is **NOT SEALED**. Mandatory next steps: complete source-to-final parity against accepted Round 6A–6J, applicable accepted cross-round amendments/corrections and R1–R30; then freeze this exact V3 file and run the comprehensive adversarial matrix plus pairwise contradiction review. Any unresolved material blocker prevents seal. Until both gates pass: Round 6 remains reopened, Round 7 remains blocked, implementation remains unauthorized.